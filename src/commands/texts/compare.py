...
async def run(ctx, user, username1, username2):
    ...
    if era_string:
        text_bests1 = await users.get_text_bests_time_travel(username1, universe, user, race_stats=True, wpm=wpm_metric, text_pool=text_pool)
        text_bests1 = [(text["text_id"], text["wpm"], text["number"], text["timestamp"], text["accuracy"], text["points"]) for text in text_bests1]
        text_bests2 = await users.get_text_bests_time_travel(username2, universe, user, race_stats=True, wpm=wpm_metric, text_pool=text_pool)
        text_bests2 = [(text["text_id"], text["wpm"], text["number"], text["timestamp"], text["accuracy"], text["points"]) for text in text_bests2]
