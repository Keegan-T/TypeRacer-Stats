...
command = {
    "name": "textbestsline",
    "aliases": ["tbl"],
    "description": "Displays a graph of user's texts best average over time",
    "parameters": "[username] <username_2> ... <username_10> <start_date> <end_date>",
    "usages": [
        "textbestsline rektless",
        "textbestsline clergy 2023-09-01",
        "textbestsline keegant 2024-01-01 2024-06-01",
        "textbestsline rektless clergy charlieog keegant xanderec",
    ],
}


class TextBestsLine(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @commands.command(aliases=command["aliases"])
    async def textbestsline(self, ctx, *args):
        ...
        async with locks.line_lock:
            ...
            await run(ctx, user, result, column="text_best_average")


async def setup(bot):
    await bot.add_cog(TextBestsLine(bot))
